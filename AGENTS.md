...
**App:** C24 Club — an Omegle-style video chat app where users earn reward minutes by chatting and can redeem them in a reward store.
...
- `app/(tabs)/chat.tsx` — Video Chat placeholder screen
- `app/(tabs)/rewards.tsx` — Reward Store (2-col grid, category filter)
- `app/(tabs)/promos.tsx` — Promotions placeholder screen
- `app/(tabs)/profile.tsx` — Profile placeholder (Sign In / Create Account)
...
### Color Palette (C24 Club Theme)

Dark-themed, high-energy:
- **Primary BG:** `#1A1A2E` (Deep Navy)
- **Surface/Card:** `#1E1E38` (Lighter Navy)
- **Primary CTA:** `#EF4444` (Vibrant Red)
- **Secondary:** `#22C55E` (Success Green)
- **Accent:** `#FACC15` (Yellow — used for minutes/highlights)
- **Text Primary:** `#FFFFFF`
- **Text Secondary:** `#A1A1AA`
- **Dividers:** `#2A2A4A`
- **Inactive:** `#71717A`

Most screens use `StyleSheet.create` with inline hex colors (not NativeWind classes) for precision.
...
- `utils/flatten-style.ts` — `flattenStyle()` helper to avoid style array issues on web (flattens on web, passes through on native)
...
- **Auth:** Login / Sign Up screens and backend integration
- **Video Chat:** WebRTC or Agora/Twilio integration with camera/microphone permissions
- **Minutes System:** Backend to track and persist earned minutes
- **Rewards Redemption:** Real redemption flow with backend
- **Promos Feed:** Full community promotions feed