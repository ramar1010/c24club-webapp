...
const ChallengeEarningsModal = ({ onClose, onSuccess }: ChallengeEarningsModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-black/80 flex items-center justify-center px-4" onClick={onClose}>
      <div
        className="bg-neutral-900 border border-white/10 rounded-2xl p-5 w-full max-w-sm relative max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <div className="text-center mb-4">
          ...
          <h2 className="text-white font-bold text-lg">Challenge Earnings</h2>
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-3 mb-4 space-y-1.5">
          <div className="flex justify-between items-center">
            <span className="text-white/50 text-xs">🏆 Total Challenge Cash Earned</span>
            <span className="text-emerald-400 font-bold text-sm">${totalEarned.toFixed(2)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-white/50 text-xs">💸 Already Cashed Out</span>
            <span className="text-white/60 font-bold text-sm">${totalCashedOut.toFixed(2)}</span>
          </div>
          <div className="border-t border-white/10 pt-1.5 flex justify-between items-center">
            <span className="text-white/50 text-xs">💰 Available to Cash Out</span>
            <span className="text-emerald-400 font-bold text-lg">${availableCash}</span>
          </div>
        </div>
        ...
        {cashEarnings.length > 0 && (
          <div className="mb-4">
            <h3 className="text-white/60 text-[10px] font-bold uppercase tracking-wider mb-2">Earned Rewards</h3>
              ...
              {cashEarnings.map((e: CashEarning) => (
                <div key={e.id} className="flex items-center justify-between bg-white/5 rounded-lg px-3 py-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white text-xs font-bold truncate max-w-[140px]">{e.challenge_title}</span>
                  </div>
                  <span className="text-emerald-400 text-xs font-black">${e.cash_amount}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {cashEarnings.length === 0 && (
          <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-4 text-center">
            <p className="text-white/40 text-sm font-bold">No cash rewards yet</p>
          ...
          <>
            ...
            {!canCashout && giftedMinutes < minCashout && (
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 mb-4">
                <p className="text-amber-400 text-xs font-bold">⚠️ Minimum not reached</p>
            ...
            {hasPending && (
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 mb-4">
                <p className="text-amber-400 text-xs font-bold">⏳ Pending cashout</p>
                <p className="text-white/50 text-[11px]">
