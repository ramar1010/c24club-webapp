...
async function sendBountyNotifications(
  ...
  try {
    ...
    console.error("[iap-purchases] DM insert error:", err?.message);
  }
  ...
  try {
    ...
    console.log("[iap-purchases] Bounty push:", { femaleId, result });
  } catch (err: any) {
    console.error("[iap-purchases] Push error:", err?.message);
  }
}
