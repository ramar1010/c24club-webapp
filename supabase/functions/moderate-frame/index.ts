...
serve(async (req) => {
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          },
          {
            role: "user",
            content: [
              { type: "text", text: "Classify this video frame. Output JSON only." },
    ...
    // Log report (no ban — sticky blur on client side handles enforcement)
    await supabaseAdmin.from("user_reports").insert({
      reporter_id: user.id,
      reported_user_id,
      reason: "AI_AUTO_MODERATION",
      details: `Auto-detected: ${topReason} (score: ${(allScores as any)[topReason].toFixed(2)})`,
    });

    // Increment strike counter (tracks for client blur stickiness; no auto-ban)
    ...
    if (mm?.id) {
      ...
      await supabaseAdmin.from("member_minutes").insert({ user_id: reported_user_id, nsfw_strikes: newStrikes });
    }

    console.log(`moderate-frame flagged ${reported_user_id} (${topReason}) — strike #${newStrikes}, no ban`);

    return new Response(
      JSON.stringify({ flagged: true, reason: topReason, score: (allScores as any)[topReason], strikes: newStrikes, banned: false }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 200 }
    );
  } catch (err: any) {
