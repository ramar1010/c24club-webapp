...
BEGIN
  ...
  IF p_is_renewal THEN
    v_minutes := 100; v_source := 'renewal'; v_tier_label := 'VIP Renewal';
  ELSIF p_tier = 'premium' THEN
    v_minutes := 249; v_source := 'premium'; v_tier_label := 'Premium VIP';
  ELSIF p_tier = 'basic' THEN
    v_minutes := 75; v_source := 'basic'; v_tier_label := 'Basic VIP';
  ELSE
  ...
  -- Send DM from owner to female letting her know she earned a bounty
  BEGIN
    SELECT COALESCE(name, 'Someone') INTO v_male_name FROM members WHERE id = p_male_id;

    SELECT id INTO v_conv_id
    FROM conversations
    WHERE (participant_1 = v_owner_id AND participant_2 = v_female_id)
       OR (participant_1 = v_female_id AND participant_2 = v_owner_id)
    LIMIT 1;

    IF v_conv_id IS NULL THEN
      INSERT INTO conversations (participant_1, participant_2)
      VALUES (v_owner_id, v_female_id)
      RETURNING id INTO v_conv_id;
    END IF;

    v_dm_body := '🎉 Bounty Win! ' || v_male_name || ' just subscribed to ' || v_tier_label ||
      ' — you earned ' || v_minutes || ' gifted minutes! 💸' || E'\n\n' ||
      'Cash out anytime: tap "My Rewards" → "Cashout Minutes" to convert your gifted minutes to PayPal cash.';

    IF v_streak_count = 3 THEN
      v_dm_body := v_dm_body || E'\n\n' || '🔥 STREAK BONUS! 3 converts in 7 days — +500 bonus gifted minutes added!';
    END IF;

    INSERT INTO dm_messages (conversation_id, sender_id, content)
    VALUES (v_conv_id, v_owner_id, v_dm_body);
