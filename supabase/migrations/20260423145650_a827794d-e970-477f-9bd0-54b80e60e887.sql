CREATE TABLE IF NOT EXISTS public.member_welcome_dm_log (
  user_id uuid PRIMARY KEY,
  sent_gender text NOT NULL,
  sent_at timestamp with time zone NOT NULL DEFAULT now()
);

ALTER TABLE public.member_welcome_dm_log ENABLE ROW LEVEL SECURITY;
...
DROP TRIGGER IF EXISTS trg_auto_dm_welcome ON public.members;

CREATE TRIGGER trg_auto_dm_welcome
AFTER INSERT OR UPDATE OF gender ON public.members
FOR EACH ROW
