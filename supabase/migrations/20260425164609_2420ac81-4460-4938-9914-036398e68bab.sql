-- Add account_type to reddit_task_submissions
ALTER TABLE public.reddit_task_submissions
  ADD COLUMN IF NOT EXISTS account_type text NOT NULL DEFAULT 'fresh'
  CHECK (account_type IN ('fresh', 'aged'));

-- Replace submit_reddit_task to accept p_account_type
CREATE OR REPLACE FUNCTION public.submit_reddit_task(
  p_code text,
  p_worker_name text,
  p_posted_url text,
  p_variant_index integer DEFAULT NULL,
  p_account_type text DEFAULT 'fresh'
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_account_type text;
BEGIN
  v_account_type := CASE WHEN lower(coalesce(p_account_type,'fresh')) = 'aged' THEN 'aged' ELSE 'fresh' END;
  ...
  INSERT INTO public.reddit_task_submissions (task_id, worker_name, posted_comment_url, variant_index, variant_text_hash, account_type)
  VALUES (v_task.id, NULLIF(trim(p_worker_name), ''), trim(p_posted_url), p_variant_index, v_variant_hash, v_account_type);
