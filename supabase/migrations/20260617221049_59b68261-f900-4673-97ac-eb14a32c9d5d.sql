...
DECLARE
  ...
  v_existing UUID;
BEGIN
  -- Renewals no longer award bounty minutes (female earned the first payment only)
  ...
  IF lower(coalesce(p_tier, '')) = 'premium' THEN
    v_minutes := 500;  -- $10.00 at $0.02/min — matches Premium VIP first payment
    v_source := 'premium';
  ELSIF lower(coalesce(p_tier, '')) = 'basic' THEN
    v_minutes := 125;  -- $2.50 at $0.02/min — matches Basic VIP first payment
    v_source := 'basic';
  ELSE
